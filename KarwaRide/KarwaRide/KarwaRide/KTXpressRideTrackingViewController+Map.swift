...
extension KTXpressRideTrackingViewController: GMSMapViewDelegate {
    internal func addMap() {
        
        let camera = GMSCameraPosition.camera(withLatitude: (self.vModel?.rideServicePickDropOffData?.pickUpCoordinate?.latitude)!, longitude: (self.vModel?.rideServicePickDropOffData?.pickUpCoordinate?.longitude)!, zoom: 12.0)
        ...
        let padding = UIEdgeInsets(top: 0, left: 100, bottom: 100, right: 100)
