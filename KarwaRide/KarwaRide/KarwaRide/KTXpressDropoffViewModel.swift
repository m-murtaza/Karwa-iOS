...
//            del?.setETAString(etaString: "")
          ...
          if currentBookingStep == BookingStep.step1
          {
               if updateMap
               {
                  if(isFirstZoomDone)
                  {
                      (self.delegate as! KTXpressDropoffViewModelDelegate).updateLocationInMap(location: location, shouldZoomToDefault: false)
                  }
                  else
                  {
                      (self.delegate as! KTXpressDropoffViewModelDelegate).updateLocationInMap(location: location, shouldZoomToDefault: true)
